...
subroutine b1_execute_3d(mesh_dims, power_field1, power_field2, temperature_field, field_size) &
  ...
  ! 添加更详细的调试输出
  write(*,*) "Fortran: 接收到的数据:"
  write(*,*) "  mesh_dims = ", mesh_dims
  write(*,*) "  field_size = ", field_size
  
  half_size = field_size
  
  ! 处理数据
  do i = 1, half_size
    call random_number(random)
    ! 0-63 的数据
    power_field1(i) = 100.0D0 + 10 * random + 10 * times
  end do
  
  do i = 1, half_size
    call random_number(random)
    ! 64-127 的数据
    power_field2(i) = 500.0D0 + 10 * random + 10 * times
  end do
  
  write(*,*) "Fortran: 计算完成"
  write(*,*) "  power_field1(1) = ", power_field1(1)
  write(*,*) "  power_field2(1) = ", power_field2(1)
  write(*,*) "  power_field1(half_size) = ",half_size, power_field1(half_size)
  write(*,*) "  power_field2(half_size) = ",half_size, power_field2(half_size)
