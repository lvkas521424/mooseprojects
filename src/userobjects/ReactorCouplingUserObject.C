...
void
ReactorCouplingUserObject::initialize()
{
  // 初始化工作，如果需要
...
// 执行第一个燃耗步
bool
ReactorCouplingUserObject::executeFirstStep()
{
...
// 执行后续燃耗步
bool
ReactorCouplingUserObject::executeSubsequentStep()
{
...
// 执行第一次中子学计算
void
ReactorCouplingUserObject::executefirstNeutronics()
{
  // 检查中子学多应用程序是否存在
...
// 执行后续中子学计算
void
ReactorCouplingUserObject::executesubsquentNeutronics()
{
  // 检查中子学多应用程序是否存在
  if (!_fe_problem.hasMultiApp(_neutronics_app_name))
  {
...
// 执行第一次核热耦合计算
bool
ReactorCouplingUserObject::executefirstCoupled()
{
  try
  {
    std::cout << "\n====== 开始固定点迭代 ======" << std::endl;
...
// 执行后续核热耦合计算
bool
ReactorCouplingUserObject::executesubsquentCoupled()
{
  // 检查多应用程序是否存在
...
// 更新Fortran程序中的燃耗步信息
void
ReactorCouplingUserObject::updateFortranBurnupStep()
{
